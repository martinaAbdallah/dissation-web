<?php
  $page_title = 'Edit Market Group';
  require_once('includes/load.php');
  // Checkin What level user has permission to view this page
   page_require_level(1);
?>
<?php
  $e_group = find_by_id('market_groups',(int)$_GET['id']);
  if(!$e_group){
    $session->msg("d","Missing Group id.");
    redirect('market_group.php');
  }
?>
<?php
  if(isset($_POST['update'])){
   ...
   if(empty($errors)){
        ...
        $query  = "UPDATE market_groups SET ";
        ...
        $result = $db->query($query);
         if($result && $db->affected_rows() === 1){
          ...
          redirect('edit_market_group.php?id='.(int)$e_group['id'], false);
        } else {
          //failed
